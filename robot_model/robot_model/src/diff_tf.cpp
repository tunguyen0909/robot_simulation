...
void Odommetry_cals::velCallback(const geometry_msgs::Twist& vel){
    ...
    double delta_x = (cos(delta_heading) * cos(heading_) - (-sin(delta_heading)) * sin(heading_)) * linear_vel_x_* vel_dt_; //m
    double delta_y = (cos(delta_heading) * sin(heading_) + (-sin(delta_heading)) * cos(heading_)) * linear_vel_x_ * vel_dt_; //m

    x_pos_ += delta_x;
    y_pos_ += delta_y;
    heading_ += delta_heading;
    if (heading_ > _2_pi){
        heading_ = heading_ - _2_pi;
    }
    if (heading_ < 0){
        heading_ = heading_ + _2_pi;
    }
